...
| 策略0 | 已实现并测试 | 高隐蔽相对时序，用两个连续间隔大小关系表示 0/1；已接入真实 `iperf -u` 业务流代理，低速短消息验证通过。 |
| 策略1 | 已实现并测试 | 高容量排序时序，用三个间隔排序表示 2 bit；已接入真实 `iperf -u` 业务流代理，使用更大间隔和重复挂载增强抗轻微丢包能力。 |
...
experiments/run_interactive_closed_loop.py
                              中期交互式闭环演示：自动启动拓扑、业务流、INT、规则选策略和收发重组
...
- `docs/implementation_guide.md`：当前实现原理，包括 P4、INT、路径调度和六策略真实 UDP 业务流挂载。
- `docs/midterm_validation.md`：中期审核建议验证流程。
