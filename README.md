...
h1: 10.0.1.2/24，默认网关 10.0.1.1
h2: 10.0.2.2/24，默认网关 10.0.2.1
...
当前拓扑按标准三层跨网段转发实现：h1/h2 只 ARP 各自默认网关，P4 交换机通过 `arp_request_table` 回应网关 ARP；IPv4 转发由 `ipv4_lpm` 执行目的 MAC/源 MAC 重写、TTL 递减和出端口选择。多路径模式覆盖出端口后，P4 在 egress 根据实际链路端口重写 s1-s2 间链路 MAC，因此不再依赖同网段直连或静态主机 ARP。
...
为解决长输入过慢和长输入失败的问题，五窗口默认发送路径已经做了两点调整：第一，默认分段从早期 6 字节提高到 16 字节，避免长文本被切成过多 segment，同时保留较细的多路径/策略切换粒度；第二，策略选择同时参考 INT 测量值和窗口5手动设置的 netem delay/loss，刚设置为丢包的链路会立即被避开，除非所有链路都不健康才使用策略2可靠模式。
...
截至当前版本，中期答辩建议按下面口径介绍系统：

1. **拓扑与业务流**：系统采用 `h1 -- s1 -- 三条并行链路 -- s2 -- h2`，h1 为 `10.0.1.2/24`，h2 为 `10.0.2.2/24`，P4 交换机按标准三层转发语义重写源/目的 MAC、递减 TTL 并选择路径。真实业务流使用 UDP `iperf` 模拟，隐蔽数据通过终端侧代理挂载到这些业务包上，不再单独构造一条与业务无关的策略流。
2. **INT 链路状态感知**：INT 暂定验证方向为 `h2 -> h1`。s2 按采样间隔从真实业务流中抽取业务包，在 IPv4 头后插入 4 字节 compact INT shim 和 probe data；s1 在转发给 h1 前剥离 INT、恢复原始协议字段和 IPv4 长度，同时本地生成 UDP/50100 报告给 h1 侧解析程序。默认采样间隔可配置，当前演示按约 500 ms 采样，链路状态窗口约 1 s 刷新。
3. **INT 丢包率计算**：业务链路丢包率不直接用 `trace_id` 缺口作为主指标，而是比较连续 INT 报告中“源交换机出端口累计包数差”和“下一跳/终点交换机入端口累计包数差”，即 `(sent_delta - recv_delta) / sent_delta`。展示窗口使用最近约 5 s 滑动统计，避免瞬时样本太少导致丢包率跳动。
4. **六策略承载方式**：策略0/1使用业务包时序和 2 字节临时同步标签；策略2/4使用 IPv4 Identification 字段承载加密后的自描述符号；策略3通过业务包长度区间和同步小头承载符号；策略5通过多路径路径序列承载真实隐蔽比特，IP-ID 只做策略、路径和顺序提示。
5. **IP-ID 相同或误撞处理**：P4 不会因为两个包 IP-ID 相同而丢弃；系统通过避免 IP 分片规避 IPv4 重组风险。接收端也不会把 IP-ID 作为唯一身份，而是结合 `strategy_id`、序号、frame/block、magic、长度、CRC/认证字段和会话窗口做多级校验；普通业务包即使偶然撞上相似 IP-ID，也只会成为候选并在校验失败后被过滤，不影响业务 payload 转发。
6. **策略选择状态**：当前阶段暂未接入 PPO，使用规则选择器作为中期基线。规则输入来自 INT 的 delay、jitter、loss、带宽和队列状态，输出为每个 chunk/segment 的策略、路径集合和必要冗余参数。后续 PPO 只需要输出同格式策略计划即可替换规则选择器。
7. **实验图与数据**：丢包-正确率实验图和原始 JSON 数据保存在 `experiments/results/loss_accuracy_figures/`，当前保留 PNG 和 JSON，不保留 PDF。中文图包括单链路、双链路、三链路同时丢包以及三场景对比图，适合直接放入 PPT 或论文。

中期答辩时最重要的边界是：**已经完成除 PPO 之外的原型闭环**，即拓扑可通、业务流不中断、INT 能持续测三条链路状态、六个策略能够挂载在真实 UDP 业务包上、接收端能够识别策略并恢复隐蔽数据；PPO 是下一阶段把规则选择器替换为学习型决策器的工作。
